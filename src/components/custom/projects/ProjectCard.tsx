...
interface ProjectCardProps {
  ...
  repoKey?: keyof typeof GitHubLinks;
  downloadKey?: keyof typeof DownloadLinks;
  liveKey?: keyof typeof LiveLinks;
}
...
const ProjectCard: FC<ProjectCardProps> = ({
  ...
  const githubHref = GitHubLinks[repoKey];
  const downloadHref = DownloadLinks[downloadKey] || "";
  const liveHref = LiveLinks[liveKey] || "";

  return (
    <Card className={`bg-transparent rounded-xl border border-card-foreground/20 `}>
      ...
      <CardContent className="w-fit">
        ...
        <div className="flex flex-col gap-x-10 mt-5 2xl:flex-row">
          ...
          {imgSrc && (
            <Image className="mt-4 2xl:mt-0 w-72 h-72" src={imgSrc || ""} alt="" />
          )}
        </div>
      </CardContent>
